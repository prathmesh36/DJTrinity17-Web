...
?>		  
          <form class="col s12" action="eventadd.php" method="POST"  enctype="multipart/form-data">
            <div class="row">
              ...
              <div class="input-field col s12">
                <input id="oneline" name="oneline" type="text" class="validate">
              ...
              <div class="input-field col s12">
                <input id="coname" name="coname" type="text" class="validate" required>
                <label for="coname" class="black-text">Coordinator Name</label>
              </div> 
              <div class="input-field col s12">
                <input id="conum" name="conum" type="text" class="validate" required>
              ...
              <div class="input-field col s12">
                <input id="regurl" name="regurl" type="text" class="validate">
...
?>			
    ...
    <li>
	  ...
	  <div class="col l8 offset-l2 s10 offset-s1">
 <?php
 if(isset($_POST['title']) && isset($_POST['desc']) && isset($_POST['shouri']) && isset($_POST['smini']) && isset($_POST['ehouri']) && isset($_POST['emini']) && isset($_POST['file']) && isset($_POST['edate']) && isset($_POST['cat']) && isset($_POST['pricedet']) && isset($_POST['oneline']) && isset($_POST['coname']) && isset($_POST['conum']))
 {
	 $title=$_POST['title'];
	 $desc=$_POST['desc'];
	 $shour=$_POST['shouri'];
	 ...
	 if(!empty($title) && !empty($desc) && !empty($shour) && !empty($smin) && !empty($ehour) && !empty($emin) && !empty($file) && !empty($edate) && !empty($cat) &&!empty($pricedet) && !empty($coname) && !empty($conum) && !empty($oneline))
	 {
		 $stime=$shour.':'.$smin.':'.'00';
		 $etime=$ehour.':'.$emin.':'.'00';	
		 $queryed="SELECT id FROM event ORDER BY ID DESC LIMIT 1";
				$queryed_run=$conn->query($queryed);
					if ($queryed_run && $queryed_run->num_rows)  
					{  
						$pid = (int)$queryed_run->fetch_assoc()['id']+1;  
					}
		 $posturl="http://djtrinity.in/admin/event/eventimg/".$pid.".jpg";			
		 $query="INSERT INTO event VALUES('','".$title."','".$desc."','".$edate."','".$stime."','".$etime."','".$venue."','".$cat."','".$pricedet."','".$coname."','".$conum."','".$posturl."','".$oneline."','".$regurl."')";
			 ...
			 if(isset($name))
			  {
				if(!empty($name))
				{
				   ...
				   if(($extension=='jpg' || $extension=='jpeg' || $extension=='png') && ($type=='image/jpeg' || $type=='image/png'))
						{
						$queryed="SELECT id FROM event WHERE title='".$title."'";
									 $queryed_run=$conn->query($queryed);
									if ($queryed_run && $queryed_run->num_rows)  
									{  
										$eid = $queryed_run->fetch_assoc()['id'];  
									}							
