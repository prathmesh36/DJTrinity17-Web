...
?>			
    ...
    <li>
	  ...
	  <div class="col l8 offset-l2 s10 offset-s1">
 <?php
 if(isset($_POST['title']) && isset($_POST['desc']) && isset($_POST['shouri']) && isset($_POST['smini']) && isset($_POST['ehouri']) && isset($_POST['emini']) && isset($_POST['edate']) && isset($_POST['cat']) && isset($_POST['pricedet']) )
 {
	 $title=$_POST['title'];
	 $desc=$_POST['desc'];
	 $shour=$_POST['shouri'];
	 ...
	 if(!empty($title) && !empty($desc) && !empty($shour) && !empty($smin) && !empty($ehour) && !empty($emin) && !empty($edate) && !empty($cat) && !empty($price))
	 {
		 $stime=$shour.':'.$smin.':'.'00';
		 $etime=$ehour.':'.$emin.':'.'00';		 
		 $query="UPDATE event SET title='".$title."', descrip='".$desc."', evtdate='".$edate."', stime='".$stime."', etime='".$etime."', venue='".$venue."', category='".$cat."', price='".$price."' WHERE id='".$eid."'";
