...
{
	$eid=$_GET['id'];
	$querye="SELECT * FROM event WHERE id='".mysql_real_escape_string($eid)."'";
...
?>			
 ...
 if(isset($_POST['title']) && isset($_POST['desc']) && isset($_POST['shouri']) && isset($_POST['smini']) && isset($_POST['ehouri']) && isset($_POST['emini']) && isset($_POST['file']) && isset($_POST['edate']) && isset($_POST['cat']) && isset($_POST['pricedet']) )
 {
	 ...
	 if(!empty($title) && !empty($desc) && !empty($shour) && !empty($smin) && !empty($ehour) && !empty($emin) && !empty($file) && !empty($edate) && !empty($cat) && !empty($price))
	 {
		 $stime=$shour.':'.$smin.':'.'00';
		 $etime=$ehour.':'.$emin.':'.'00';		 
		 $query="UPDATE event SET title='".mysql_real_escape_string($title)."', descrip='".mysql_real_escape_string($desc)."', evtdate='".mysql_real_escape_string($edate)."', stime='".mysql_real_escape_string($stime)."', etime='".mysql_real_escape_string($etime)."', venue='".mysql_real_escape_string($venue)."', category='".mysql_real_escape_string($cat)."', price='".mysql_real_escape_string($price)."' WHERE id='".mysql_real_escape_string($eid)."'";
					   ...
					   if(move_uploaded_file($tmp_name,$location.$eid.'.jpg'))
					   {
									 echo '<br>
										  <ul class="collection" style="opacity: 0.8;border:1px solid black;border-radius:10px;">
											<li class="collection-item active"><b>Event </b><a href="edit.php?id='.$eid.'" class="secondary-content"><i class="material-icons">send</i> <span>Edit</span></a></li>										  
