			...
			while($rows=mysql_fetch_assoc($queryed_run))
			{
			 echo '
				  <ul class="collection" style="opacity: 0.8;border:1px solid black;border-radius:10px;">
					<li class="collection-item active blue lighten-2"><b>Question '.$rows['QuestionId'].'-: </b>'.$rows['Question'].'<a href="edit.php?id='.$rows['QuestionId'].'" class="secondary-content"><i class="material-icons">send</i> <span>Edit</span></a></li>
					...
					<li class="collection-item active blue lighten-2"><div><b>Answer -: </b>'.$rows['Answer'].'</div></li>				  
				  </ul>				  
			 ';
			}
			?>		  
