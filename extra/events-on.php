...
<head>
		...
		if(isset($_GET["id"]))
		{
			...
			if(!empty($id))
			{
				$query="SELECT title,poster_url from event WHERE id='".$id."'";
				...
				echo '<b:if cond="data:blog.pageType == “index”>
						...
						<b:else/>
						<meta content="admin/event/eventimg/11.jpg" property="og:image"/>
						</b:if>
						</b:if>
						<b:if cond="data:blog.metaDescription">
						<meta expr:content="data:blog.metaDescription" property="og:description"/>
						<b:else/>
						<meta content="'.$title['title'].'" property="og:description"/>
						</b:if>';
				}
			}
		}else{
    ...
    <div class="modal fade bd-example-modal-lg" id="evtmodal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
		  <div class="modal-dialog modal-lg1">	  
			...
			<?php 
				  ...
				  echo '<div class="modal-body" style="font-size:140%;">
					...
					<div class="row"><div class="col-xs-4 text-left" style="padding-left:2%;padding-right:0px;"><span class="glyphicon glyphicon-user"></span><b>&nbsp Coordinator: </b></div><div class="col-xs-8 text-left" style="padding-left:0px;"><p id="coord"></p></div></div>					
					</div>
		...
		function loadmodal()
		{
			...
			if(url)
			{
				...
				xmlhttp.open("GET","test.php?id="+url,false);
				xmlhttp.send(null);
		...
		url=url.replace('#','');
		var filterurl=url.replace(/ /g,"_").toLowerCase();		
		xmlhttp.open("GET","test.php?id="+filterurl,false);
		xmlhttp.send(null);
		var evtdata=xmlhttp.responseText;
		var evtarr = evtdata.split("~");
		if(evtarr[1]!="undefined")
